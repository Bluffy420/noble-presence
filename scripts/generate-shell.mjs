 ...
 *   - Logo image + NB Associates text + tagline
 *   - Nav links highlight gold (#c9a84c) on hover
 *   - "Consult Us" button: navy base, gold (#c9a84c) on hover
...
.nba-logo-sub {
  ...
  letter-spacing: .16em; color: var(--nba-muted-fg);
  margin-top: 2px;
}

/* ── Desktop nav ── */
.nba-desktop-nav { display:none; align-items:center; gap:2rem; }
...
.nba-nav-link:hover {
  ...
  border-bottom-color: var(--nba-gold) !important;
...
.nba-hamburger span {
  ...
  background: var(--nba-navy); transition:transform .2s, opacity .2s;
}
.nba-hamburger.nba-open span:nth-child(1){ transform:translateY(6.5px) rotate(45deg); }
.nba-hamburger.nba-open span:nth-child(2){ opacity:0; }
.nba-hamburger.nba-open span:nth-child(3){ transform:translateY(-6.5px) rotate(-45deg); }

/* ── Mobile nav ── */
.nba-mobile-nav {
  display:none; border-top:1px solid var(--nba-border);
  background: var(--nba-ivory); font-family:var(--nba-font);
}
...
.nba-mobile-link {
  ...
  color: var(--nba-navy); text-decoration:none;
  transition: color .15s;
}
.nba-mobile-link:hover{ color: var(--nba-gold) !important; }
.nba-mobile-cta-btn {
  ...
  text-decoration:none !important; margin-top:1.25rem;
  border-radius: var(--nba-r);
  transition: background .2s, color .2s;
}
...
.nba-footer-brand-sub  { font-size:10px; text-transform:uppercase; letter-spacing:.18em; color:var(--nba-muted-fg); margin-top:4px; }
.nba-footer-brand-desc { margin-top:1.25rem; font-size:.875rem; color:var(--nba-muted-fg); line-height:1.6; max-width:320px; }
.nba-footer-heading    { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.16em; margin-bottom:1rem; color:var(--nba-fg); }
